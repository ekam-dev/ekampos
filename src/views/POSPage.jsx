...
import { IconPlus, IconNotes, IconArmchair, IconScreenShare, IconSearch, IconDeviceFloppy, IconChefHat, IconCash, IconMinus, IconNote, IconTrash, IconFilter, IconPhoto, IconFilterFilled, IconClipboardList, IconX, IconClearAll, IconPencil, IconCheck,IconSoup, IconCarrot, IconRotate, IconQrcode, IconArmchair2, IconUser } from "@tabler/icons-react"; import _ from 'lodash';
import { VITE_BACKEND_SOCKET_IO, iconStroke } from "../config/config";
import { PushSpinner,ImpulseSpinner } from "react-spinners-kit";
import { cancelAllQROrders, cancelQROrder, createOrder, createOrderAndInvoice, getDrafts, getQROrders, getQROrdersCount, initPOS, setDrafts,orderStates } from "../controllers/pos.controller";
import { payAmount,bridge,defaultTimeOut } from '../helpers/PayLib';
...
import { useMenuModifiers } from "../controllers/setmodifier.controller";
...
export default function POSPage() {
  ...
  const tp = paxTerminal?.reduce((result) => { return result })
  var ip = tp?.service_ip;
  ...
  const [payStatus, setPayStatus] = useState({ amount: '', paymentStatus: '', success: true, data: null, error: false, statusCode: '' });
  const [isCountInit, setisCountInit] = useState({isplay:false,init:parseInt(defaultTimeOut),key:0});
  // dialog: notes ref
  ...
  const slide = categories.map(items => {
    <slider>{items.name}</slider>
  })
  const clientMachine = tenantPax?.reduce(i=>i)
  ...
  const [modifierList, modifierState] = useState([]);
  const [modifierItemPro, modifierProItem] = useState([]);
  const [itemPrice,setItemPrice] = useState();

  const requireModifierHandler = async(itemData) => {
    const response = await fetch(VITE_BACKEND_SOCKET_IO + `/api/v1/modifiers/modifiers-set/get-menu-modifiers/${itemData.id}`)
    const result = await response.json()
    modifierState(result)
    modifierProItem(itemData.title)
    setItemPrice(itemData.price);
    document.getElementById('modal-modifier-set').showModal() 
  }
  const [itm, itemHandler] = useState([]);
  const [itemCount, itemLength] = useState(1);
  const [modifierMenu, setModifierMenu] = useState([]);
  
  const items = [];
  
  const modiflerItemHandler = async (e, data, modifier) => {
    let modiferItemsLength = 0;
    let total = 0;
    let elClass = 'bg-sky-200';
    if (e.target.parentElement.classList.contains(elClass)) {
          e.target.parentElement.classList.remove(elClass)
    } else {
      e.target.parentElement.classList.add(elClass)
    }
      
        const localStore = localStorage.getItem('mod_itemid_' + modifier.mid)
        const localItems = JSON.parse(localStore);
        setModifierMenu(data.menu_id);
    if (e.target.parentElement.classList.contains(elClass)) {
      if (localItems) {
        if (data.is_required) {
          if (localItems.length >= data.max_allow) {
              e.target.parentElement.classList.remove(elClass)
              let el = localItems.slice(-1)[0];
              let elm = document.getElementById('moditem_' + el.id)
              elm.classList.remove(elClass)
              localItems.pop();
              total = parseFloat(itemPrice) - parseFloat(el.price)
              total = total + parseFloat(modifier.price)
              setItemPrice(total.toFixed(2))
              localItems.push({ name: modifier.name, id: modifier.v, price: modifier.price, isrequired: data.is_required, modifierid: modifier.mid })
              localStorage.setItem('mod_itemid_' + modifier.mid, JSON.stringify(localItems));
              let filter_Items = itm.pop();
              itemHandler([...itm, { name: modifier.name, id: modifier.v, price: modifier.price, isrequired: data.is_required, modifierid: modifier.mid }]);
              e.target.parentElement.classList.add(elClass);
            } else {
              localItems.push({ name: modifier.name, id: modifier.v, price: modifier.price, isrequired: data.is_required });
              localStorage.setItem('mod_itemid_' + modifier.mid, JSON.stringify(localItems));
              itemHandler([...itm, { name: modifier.name, id: modifier.v, price: modifier.price, isrequired: data.is_required, modifierid: modifier.mid }]);
              total = parseFloat(itemPrice) + parseFloat(modifier.price)
              setItemPrice(total.toFixed(2))
            }
        } else {
          localItems.push({ name: modifier.name, id: modifier.v, price: modifier.price, isrequired: data.is_required,modifierid:modifier.mid });
        ...
        localStorage.setItem('mod_itemid_' + modifier.mid, JSON.stringify([{ name: modifier.name, id: modifier.v, price: modifier.price, isrequired: data.is_required }]))
        itemHandler([...itm, { name: modifier.name, id: modifier.v, price: modifier.price, isrequired: data.is_required, modifierid: modifier.mid }])
        total = parseFloat(itemPrice) + parseFloat(modifier.price)
        setItemPrice(total.toFixed(2))
      }   
      ...
      if (localItems) {
        let filterItems = localItems.filter(arr => arr.id !== modifier.v)
        let filter_Items = itm.filter(arr => arr.id !== modifier.v);
        itemHandler(filter_Items);
        localStorage.setItem('mod_itemid_' + modifier.mid, JSON.stringify(filterItems));
        e.target.parentElement.classList.remove('bg-sky-200');
        total = parseFloat(itemPrice) - parseFloat(modifier.price)
        setItemPrice(total.toFixed(2))
      }
    }  
  ...
  const btnModifierHandler = async () => {
    let price = 0;
    let selectedVariantId = null;
    const selectedAddonsId = [];  
    const selectedItem = menuItems.find((item) => item.id == modifierMenu);
    const isRequired = selectedItem?.isrequired.split(',').filter(arr => arr == 1).length;
    const requiredItems = itm.filter(arr => arr.isrequired !== 0);
    var itemValidate;
    if (isRequired > isChecked) {
      selectedItem?.modifier_setid.split(',').map((mod, index) => {
        localStorage.removeItem('mod_itemid_' + mod);
        if (requiredItems[0] != undefined) {
          let v = [...new Map(requiredItems.map(item => [item['modifierid'], item])).values()]
          setChecked(v.length)
          itemValidate = v.length >= isRequired ? true : false
        }
      })
      itemValidate != true ? toast.error(`You Must Choose Required Item!`) : '';
      if (itemValidate != true) {
        document.querySelector('#modal-modifier-set').setAttribute('open','')
        return
      }
    ...
    if (itm.length > 0) {
      selectedModifierItems = itm.map((item, index) => {
        let itemprice = (isNaN(item.price) || item.price == null || item.price == '') ? 0 : item.price;
        price = price + parseFloat(itemprice)
        return { price : parseFloat(item.price),title:item.name,id:item.id};   
      }) 
    }
    const itemCart = {...selectedItem, price: price, variant_id: selectedVariantId, variant: selectedVariant, addons_ids: selectedAddonsId, addons: selectedAddons,selectedModifierItems}
    addItemToCart(itemCart)
    itemHandler([])
    itemValidate = false;
    setChecked(0)
    let check = document.querySelectorAll('.bg-sky-200')
    check.forEach((item, i) => { item.classList.remove('bg-sky-200') });
    document.querySelector('#modal-modifier-set').close()
  }

  const btnModifierHandlerClose = async (init) => {
      itemHandler([])
      setChecked(0)
      init.map((e) => {
        localStorage.removeItem('mod_itemid_'+e.modifierset_id)
      })
      let check = document.querySelectorAll('.bg-sky-200')
      check.forEach((item, i) => { item.classList.remove('bg-sky-200') });
  }
  ...
  function addItemToCart(item) {
    if (item?.modifier_setid) {
      item?.modifier_setid.split(',').map((x, y) => {
        localStorage.removeItem('mod_itemid_' + x)
      })
    }
  ...
  const btnApplySwiperCategoryFilter = (category) => {

    const selectedCategoryFromDropdown = category.id || "";
    setState({
      ...state,
      selectedCategory: selectedCategoryFromDropdown
    })
  ...
  const btnPaxPay = async () => {
    try {
      if(cartItems) {
        ...
        bridge({ ip: clientMachine[0]?.service_ip, port: clientMachine[0]?.service_port });
        setPayStatus({ amount: Number(cartItems[0]?.price).toFixed(2) })
        console.log(cartItems[0])
        setisCountInit({isplay:true,init:parseInt(defaultTimeOut),key:0});
        const p = parseInt('0.01');//cartItems[0]?.price * 100
        //const p = cartItems[0]?.price * 100;
        payAmount({ amount:  p}, function (response) {
          console.log(response)
          if (response.ResponseCode === '100002') {
            setPayStatus({ paymentStatus: 'payment cancel' ,error:true,statusCode :response.ResponseCode})
          } else if (response.ResponseCode === '100001') {
            setPayStatus({ paymentStatus: 'time out' ,error:true,statusCode :response.ResponseCode})
          } else if (response.ResponseCode === '100003') {
            setPayStatus({ paymentStatus: 'AMOUNT INVALID' ,error:true,statusCode :response.ResponseCode})
          } else if (response.ResponseCode == '000100') {
            setPayStatus({ paymentStatus: 'DECLINE' })
          } else if (response.ResponseCode === '000000') {
            setPayStatus({ paymentStatus: 'Payment Successfully Done', error: false, statusCode: response.ResponseCode })
            setTimeout(() => { btnPayAndSendToKitchen() },3000);
          } else if (response.ResponseMessage == undefined) {
            setPayStatus({ paymentStatus: 'Payment Failed! Please Try Again',error:true ,statusCode :response.ResponseCode})
          }
          setTimeout(() => { payModalClose() }, 3000)
          const rs = orderStates(response);
          console.log(rs)
        });
        ...
        document.getElementById('modal-pax-pay').showModal();
      }
    } catch (error) {
   ...
   const timeoutEnd = () => {
     setPayStatus({ paymentStatus: 'time out', error: true, statusCode: 10000 })
     setTimeout(() => { payModalClose();setisCountInit({isplay:false,init:parseInt(defaultTimeOut),key:1}); }, 3000)
     //setisCountInit({isplay:false,init:parseInt(defaultTimeOut)});
   }
  ...
  const btnPayAndSendToKitchen = async () => {
    try {
      ...
      toast.loading("Please wait...");
      const res = await createOrderAndInvoice(cartItems, deliveryType, customerType, customer, tableId, state.itemsTotal, state.taxTotal, state.payableTotal, state.selectedQrOrderItem, state.selectedPaymentType);
      toast.dismiss();
      if(res.status == 200) {
        const data = res.data;
        toast.success(res.data.message);
  ...
  const payModalClose = () => {
      setPayStatus({ amount:'', paymentStatus:'',success:true,data:null,error:false ,statusCode:''})
      document.getElementById('modal-pax-pay').close();
  }
  ...
  return (
    <Page className='px-4 py-3 flex flex-col min-h-0'>
      ...
      <div className='mt-4 md:h-[calc(100vh-136px)] flex flex-col-reverse md:flex-row gap-4'>
        ...
        <div className='border border-restro-border-green-light rounded-2xl h-full md:w-[70%] overflow-y-auto'>
          ...
          <div className="flex gap-2 bg-white/40 backdrop-blur sticky top-0 w-full z-10 px-4 py-3 rounded-t-2xl">
            ...
            <button onClick={()=>{btnClearSelectedCategory()}} className="rounded-lg hover:bg-gray-200 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-gray-200 text-gray-500">All</button>
          ...
          {/* list */}
          <div className='grid grid-cols-6 gap-4 w-full z-0 px-4 pb-4 rounded-b-1sx'>
            <div className='row-span-12 grid-cols-4'>
                <Swiper
                  direction='vertical'
                  spaceBetween={10}
                  slidesPerView={2}
                  ...
                  >
                  {
                    categories.map(category => {
                      return (<SwiperSlide key={ category.id} style={{backgroundColor:category.color_code}} className='bg-slate-600 justify-center flex justify-items-center text-white font-bold py-10 capitalize rounded-md cursor-pointer' onClick={()=>{btnApplySwiperCategoryFilter(category);}} >{category.title}</SwiperSlide>)
                    })
                  }
                </Swiper>
            </div>
            ...
            const {title, id, price, image, category_id, category_title,color_code, addons, variants,isrequired,is_multiselect} = menuItem;

            const imageURL = image ? getImageURL(image) : null;
            const hasVariantOrAddon = variants?.length > 0 || addons?.length > 0;
            const isRequiredMode = isrequired ? isrequired?.split(','): [];
            return <div style={{backgroundColor:color_code}} className='bg-slate-600 border border-restro-border-green-light rounded-md p-2 flex gap-2 cursor-pointer' key={i} onClick={()=>{
                      if(hasVariantOrAddon) {
                        btnOpenVariantAndAddonModal(id);
                      }
                      else if (isRequiredMode.length > 0) {
                         requireModifierHandler(menuItem)
                      }
                      else {
              ...
              <div>
                ...
                <div className='mt-2'>
                    {/* <button  className='rounded-full px-4 py-1 bg-restro-green hover:bg-restro-green-dark transition active:scale-95 text-white flex items-center justify-center'>
                      Add
                    </button> */}
                </div>
              </div>
        ...
        <div className='border border-restro-border-green-light rounded-2xl h-full md:w-[30%] relative flex flex-col'>
          ...
          <div className='flex-1 flex flex-col gap-4 overflow-y-auto px-4 pb-36'>
            ...
            {cartItems?.map((cartItem, i)=>{
              const { quantity, notes, title, price, variant, addons, selectedModifierItems } = cartItem;
              ...
              return <div key={i} className='text-sm border border-restro-border-green-light rounded-lg p-2 relative'>
                ...
                {(selectedModifierItems && selectedModifierItems?.length > 0 ) && <p className="mt-1 text-xs text-gray-400">Modifiers: {selectedModifierItems?.map((items)=>(`${items.title}`))?.join(", ")}</p>}
      ...
      <dialog id="modal-modifier-set" className="modal modal-bottom sm:modal-middle h-screen">
        <div className="modal-box" style={{overflowY:'hidden'}}>
          <div className='-m-6 bg-lime-500 p-3 fixed w-full'>
            <h3 className="font-bold text-2xl ml-5 text-white first-letter:uppercase font-sans">{ modifierItemPro} </h3>
          </div>
          

          <div className="mt-20 container" style={{ maxHeight: '500px',overflowY:'auto',marginBottom:'100px'}}>
              {
              modifierList.map((items, index) => {   
                
                  return (
                      <div>
                            <div className='text-center mt-8'>
                              <p className='font-extrabold text-xl capitalize font-sans' key={index}>{items.modifier_name} <span className={`capitalize ${items.is_required == 1 ? 'text-red-400' : 'text-sky-600'}`}>{items.is_required == 1 ? '( required )' : '( optional )'}</span><span className='mx-2'>{ items.is_required==1 ? items.min_allow+' - ' : ''}  { items.is_required==1 ? items.max_allow  : ''}</span></p>
                            </div>
                            <div className='container mx-auto grid grid-cols-3'>
                                {
                                    items.modifier_items?.split(',').map((i, k) => { 
                                      return (<div className='bg-indigo-50 m-3 p-3 rounded-xl itm-modifi' id={`moditem_${items.modifier_items_id.split(',')[k]}`}>
                                        <span className='cursor-pointer' onClick={((e) => { modiflerItemHandler(e,items,{name:i,v:items.modifier_items_id.split(',')[k],price:items.price.split(',')[k],mid:items.modifierset_id}) }) }>{ i } {currency}{items.price.split(',')[k]}</span>
          ...
          <div className="modal-action bg-gray-100 p-3 -mb-6 -mx-6 fixed w-full bottom-5">
            <div className='mr-auto mt-2'>
              <span className='font-black text-3xl'>{ currency+itemPrice}</span>
            </div>
            <form method="dialog">
              {/* if there is a button in form, it will close the modal */}
              <button onClick={()=>{btnModifierHandlerClose(modifierList)}}  className="rounded-lg hover:bg-gray-200 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-gray-200 text-gray-500">Cancel</button>
              
            </form>
            <button onClick={()=>{btnModifierHandler()}} className="rounded-lg hover:bg-green-800 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-restro-green text-white ml-3">Save</button>
          </div>
        </div>
      </dialog>
      {/* dialog: Modifier Modal */}
        ...
        <div className="modal-box">
          ...
          <div className="modal-action flex items-center justify-center w-full">
            <form method="dialog" className='w-full'>
              {/* if there is a button in form, it will close the modal */}
              <button onClick={()=>{btnPaxPay();}} className="w-full text-center rounded-lg hover:bg-green-800 transition active:scale-95 hover:shadow-lg px-4 py-3 bg-restro-green text-white">Collect Payment & Send to Kitchen</button>
            </form>
          </div>
        </div>
      </dialog>
      ...
      {/* dialog: print-token */}
      <dialog id="modal-pax-pay" className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          <h3 className="font-bold text-3xl text-center">Make Payment</h3>

          <div className="my-8 mx-auto w-fit">
            <div className="flex items-center gap-2">
              {
                () => {
                  payStatus.statusCode === '000000' ?
                    <div className="w-12 h-12 flex items-center justify-center bg-restro-green text-white rounded-full">
                      <IconSoup />
                    </div>
                    : '';
            ...
            <p className="mt-4 text-3xl text-center capitalize">{payStatus.error ? '' : <span>total amount <span className='font-black'>{payStatus.amount}</span></span>}</p>
            <div className='my-6 flex justify-center capitalize items-center text-lg font-bold'>
               {payStatus.error ? '' : <span className='flex items-center'>payment to proceed <span className='mx-2'><ImpulseSpinner
                            size={30}
                            color="#686769"
                loading={!payStatus.paymentStatus} /></span></span>
              }
            ...
            <div className='flex justify-center'>
              <CountdownCircleTimer
                key={ isCountInit.key} 
                isPlaying={isCountInit.isplay}
                duration={isCountInit.init}
                size={120}
                colors={['#004777', '#F7B801', '#A30000', '#A30000']}
                colorsTime={[25,15,10,5]}
                onComplete={ timeoutEnd }
                >
            ...
            <h3 className={`capitalize font-bold text-3xl ${payStatus.statusCode == '000000' ? 'text-green-400' : 'text-red-500' }`}>{ payStatus.paymentStatus}</h3>
          </div>
