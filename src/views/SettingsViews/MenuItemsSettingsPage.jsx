...
import { addMenuItem, deleteMenuItem, useMenuItems, useModifierSet } from "../../controllers/menu_item.controller";
...
export default function MenuItemsSettingsPage() {
  ...
  async function btnAdd() {
    ...
    const list = selected.map((item, index) => { return { label: item.label, value: item.value} })
    if(!title) {
    ...
    try {
      ...
      const res = await addMenuItem(title, price, netPrice, categoryId, taxId,list);
  ...
  const btnShowUpdate = (id) => {
    navigate(`/dashboard/menu-items/${id}`);
  }
