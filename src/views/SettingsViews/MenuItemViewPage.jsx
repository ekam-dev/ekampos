...
import { addMenuItemAddon, addMenuItemVariant, deleteMenuItemAddon, deleteMenuItemVariant, removeMenuItemPhoto, updateMenuItem, updateMenuItemAddon, updateMenuItemVariant, uploadMenuItemPhoto, useMenuItem,useModifierSet } from "../../controllers/menu_item.controller";
...
export default function MenuItemViewPage() {
  ...
  const { APIURL: modifersetURL, data: modifierSet } = useModifierSet();
  const setItems = async () => { modifierSet?.map((entity, index) => { modifierList.push({ label: entity.title, value: parseInt(entity.id) }) }) }
  setItems()
  const getList = menuItem ? menuItem : [];
  useEffect(() => {
    const modifierSelected = async () => {
       let op = getList.modifier_setid?.split(',').map((list, index) => {
          return { value: parseInt(list), label: getList.modifier_title.split(',')[index],modifierid :getList.modifierid?.split(',')[index] }
       }); 
      setSelected(op)
    }
    modifierSelected()
  ...
  async function btnSave() {
    ...
    try {
      ...
      const res = await updateMenuItem(id, title, price, netPrice, categoryId, taxId,list);
      //const res = {status:203}
      if(res.status == 200) {
        await mutate(APIURL);
        toast.dismiss();
  ...
  return (
    <Page className="px-4 md:px-8 py-3 md:py-6 container mx-auto w-2/3">
      <div className="text-sm breadcrumbs">
        <ul>
          ...
          <li>
            <Link to="/dashboard/menu-items">Menu Items</Link>
          </li>
          <li>{title}</li>
        </ul>
      </div>
                ...
                (() => {
                  return (
                        ...
                        <Select
                          ...
                          placeholder="Select Modifer Set"
                        />
                      )
                  })()
              }
