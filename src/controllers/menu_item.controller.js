...
export function useModifierSet() {
  const APIURL = `/modifiers/modifiers-set/get-all-setmodifiers`;
  const { data, error, isLoading } = useSWR(APIURL, fetcher);
  return {
    data,
    error,
...
export async function addMenuItem(title, price, netPrice, categoryId, taxId,list) {
  try {
...
export async function updateMenuItem(id, title, price, netPrice, categoryId, taxId,list) {
  try {
