...
export async function createOrderAndInvoice(cart, deliveryType, customerType, customerId, tableId, netTotal, taxTotal, total, selectedQrOrderItem, selectedPaymentType) {
  try {
    const response = await ApiClient.post("/pos/create-order-and-invoice", {
      cart, deliveryType, customerType, customerId, tableId,
      netTotal, taxTotal, total, selectedQrOrderItem, selectedPaymentType
    });
    return response;
  } catch (error) {
    throw error;
...
export async function orderStates(data) {
  try {
    const response = await ApiClient.post(`/orders/order_status`,data);
    return response;
  } catch (error) {
    throw error;
  }
